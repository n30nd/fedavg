...
def federated_train(trainloaders, valloaders, testloader, config):
    ...
    for round_num in range(num_rounds):
        ...
        local_train_net_fedavg(nets, selected_clients, config, trainloaders, device=DEVICE)
...
def local_train_net_fedavg(nets, selected_clients, config, trainloaders, device='cpu'):
    for net_id in selected_clients:
        net = nets[net_id]
        net.to(device)
        train_net_fedavg(net, trainloaders[net_id], config, device=device)


def train_net_fedavg(net, trainloader, config, device):
    optimizer = optim.SGD(
